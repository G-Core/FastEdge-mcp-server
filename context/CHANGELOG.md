...
- `src/tools/api/batch-execute.ts` — after `resolveRefs`, re-run `checkAllowed` on the concrete path before dispatch. Denial aborts the batch with `policy_denied` + `denied_step` (template and resolved path) + `completed`. Segment-count matching in `matchTemplate` means an injected `/` also fails the re-check, so no separate `/`-rejection is needed.
